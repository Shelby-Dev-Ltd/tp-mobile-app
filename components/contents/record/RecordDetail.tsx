...
import { parseHHMMSSToSeconds } from "../../../helpers/duration";
...
export const RecordDetail: React.FC<RecordDetailProps> = ({ id, navigation }) => {
    ...
    const getDecision = (d: string, videoDuration: number, carCount: number, motorcycleCount: number, truckCount: number, busCount: number): Array<string> => {
        ...
        const trafficDensity = vehiclesPerHour > 100 ? "heavy" : "light";
    
        const durationMessage = `Based on a video duration of ${videoDuration} minutes:`;
        const trafficMessage = `The traffic is ${trafficDensity} with approximately ${vehiclesPerHour} vehicles per hour.`;
        ...
        switch (d) {
            case 'MIDDLE_HIGH':
                return [
                    ...
                    `${durationMessage} ${trafficMessage} This location may be suitable for various retail businesses such as shopping malls, supermarkets, or department stores. Additionally, restaurants, cafes, or entertainment centers can also thrive here due to the high traffic volume and large potential market.`
                ];
            case 'MIDDLE_LOW':
                return [
                    ...
                    `${durationMessage} ${trafficMessage} This location is suitable for establishing middle or elementary schools. With high individual mobility, accessibility for students and parents becomes easier. Additionally, tutoring centers or training centers can also function well here.`
                ];
            case 'INDUSTRIAL':
                return [
                    ...
                    `${durationMessage} ${trafficMessage} This location can be ideal for government offices, post offices, or public service centers. Due to intensive logistic activities, the availability of space for warehouses or storage can also be an important consideration.`
                ];
            case 'TOURISM':
                return [
                    ...
                    `${durationMessage} ${trafficMessage} This location is suitable for developing tourist attractions such as hotels, tourist sites, or recreational facilities. With a significant flow of tourists, hospitality businesses, restaurants, or souvenir shops can also thrive here.`
                ];
            case 'MIXED':
                return [
                    ...
                    `${durationMessage} ${trafficMessage} This location can be an interesting place for various businesses, including corporate offices, grocery stores, restaurants, or even sports and recreational facilities. The complexity of activities in this area offers opportunities for various types of businesses and services.`
                ];
            default:
                return [];
        }
